...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    <xsl:template match="/">
        <!-- Deserialize unit/sortType info passed from form, to enable index lookup.
            The format expected is: 1:prioritySort, ... -->
        <xsl:variable name="unit-nodes" as="node()*">
            <xsl:call-template name="get-unit-nodes"/>
        </xsl:variable>
        <xsl:variable name="sortType" as="xs:string">
            <xsl:choose>
        ...
        <xsl:call-template name="main">
            ...
            <xsl:with-param name="sortUserParameter" select="$inputValue2"/>
        </xsl:call-template>
