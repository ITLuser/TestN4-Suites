...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <xsl:variable name="eqId" as="xs:string">
        <xsl:choose>
            <xsl:when test="$unit-nodes[1]/@id != ''">
                <xsl:value-of select="$unit-nodes[1]/@id"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of/>
            </xsl:otherwise>
    ...
    <xsl:variable name="ppos" as="xs:string">
        <xsl:choose>
            <xsl:when test="$unit-nodes[1]/@ppos != ''">
                <xsl:value-of select="$unit-nodes[1]/@ppos"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of/>
            </xsl:otherwise>
    ...
    <xsl:variable name="eqId2" as="xs:string">
        <xsl:choose>
            <xsl:when test="$unit-nodes[2]/@id != ''">
                <xsl:value-of select="$unit-nodes[2]/@id"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of/>
            </xsl:otherwise>
    ...
    <xsl:variable name="ppos2" as="xs:string">
        <xsl:choose>
            <xsl:when test="$unit-nodes[2]/@ppos != ''">
                <xsl:value-of select="$unit-nodes[2]/@ppos"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of/>
            </xsl:otherwise>
